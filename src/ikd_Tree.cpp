...
{
    delete_criterion_param = delete_param;//删除准则、
    balance_criterion_param = balance_param;//平衡准则
    downsample_size = box_length;//下采样尺寸
...
void KD_TREE<PointType>::multi_thread_rebuild()
{
    ...
    while (!terminated)
    {
        ...
        if (Rebuild_Ptr != nullptr)
        {
            ...
            KD_TREE_NODE *new_root_node = nullptr;//创造根节点
...
void KD_TREE<PointType>::Build(PointVector point_cloud)
{
    ...
    InitTreeNode(STATIC_ROOT_NODE);//初始化STATIC_ROOT_NODE，全设为0
    BuildTree(&STATIC_ROOT_NODE->left_son_ptr, 0, point_cloud.size() - 1, point_cloud);//将这一帧点云丢入KDTree中构建
    Update(STATIC_ROOT_NODE);//构建完树后统计各种数据
    STATIC_ROOT_NODE->TreeSize = 0;
    Root_Node = STATIC_ROOT_NODE->left_son_ptr;//设置Root_Node为STATIC_ROOT_NODE->left_son_ptr，可能是头节点才这么做。
}
...
void KD_TREE<PointType>::Nearest_Search(PointType point, int k_nearest, PointVector &Nearest_Points, vector<float> &Point_Distance, float max_dist)
{
    ...
    //cout << "Search before"<<endl; 
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {   
       // cout << "Searching"<<endl; 
        Search(Root_Node, k_nearest, point, q, max_dist);
        //cout<<"Searching after"<<endl; 
    }
    else
    {   
        //cout << "Searching else"<<endl; 
        pthread_mutex_lock(&search_flag_mutex);
    ...
    //cout<<"k_found"<<k_found<<endl; 
    //  Nearest_Points.clear();  // 清空最近点向量
    //  Point_Distance.clear(); 
    
    PointVector().swap(Nearest_Points);//Nearest_Points
    vector<float>().swap(Point_Distance);
    
    //cout<<"Nearest_Points BEFORE "<<endl; 
...
template <typename PointType>
void KD_TREE<PointType>::flatten(KD_TREE_NODE *root, PointVector &Storage, delete_point_storage_set storage_type)//delete_point_storage_set 有三种：NOT_RECORD, DELETE_POINTS_REC, MULTI_THREAD_REC
{
    if (root == nullptr)
        return;
    Push_Down(root);
