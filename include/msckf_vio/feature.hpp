...
struct Feature {
  ...
  // std::map<StateIDType, Eigen::Vector3d, std::less<StateIDType>,
  // Eigen::aligned_allocator<std::pair<const StateIDType, Eigen::Vector3d> > > position_aft;
  // 3d postion of the feature in the world frame.
  Eigen::Vector3d position;
  Eigen::Vector3d position_aft;
...
bool Feature::initializePosition(
  ...
  for (auto& m : observations) {
    ...
    auto cam_state_iter = cam_states.find(m.first);//在cam状态中找到对应IMU的ID的那帧相机状态。
  ...
  //ROS_INFO("final_position(%f,%f,%f)",final_position.x(),final_position.y(),final_position.z());
  ...
  position_aft = final_position;//****得到特征点在观测的第1帧相机系下的最后优化3D坐标。c0P，
