...
struct BoxPointType     //类体素结构
{
    float vertex_min[3];
    float vertex_max[3];
};

enum operation_set   //运算操作。加减点、加减box、下采样删除、pushdown？
{
    ADD_POINT,
    DELETE_POINT,
    DELETE_BOX,
...
public:
    ...
    struct KD_TREE_NODE
    {
        PointType point;//输入的类型是PointType
        int division_axis;//分的轴
        int TreeSize = 1;//树的大小
        int invalid_point_num = 0;//无用点数量
        int down_del_num = 0;//删除的点数量
